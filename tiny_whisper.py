...
def index_select(input_tensor, dim, index):
    if type(input_tensor) == Tensor: input_tensor = tiny_Tensor(input_tensor.numpy())
    expanded_index = [slice(None)] * len(input_tensor.shape)
